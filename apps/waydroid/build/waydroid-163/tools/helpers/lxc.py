...
def generate_session_lxc_config(args, session):
    ...
    # Resolve the host-side PulseAudio socket. Upstream hardcodes
    # `$PULSE_RUNTIME_PATH/native`, but Wolf (games-on-whales) names its
    # socket differently: it exports `PULSE_SERVER=$XDG_RUNTIME_DIR/pulse-socket`
    # (see wolf docker/startup.sh) and the socket is a FILE at that path, not
    # a `pulse/native` layout. With the upstream path the bind mount silently
    # points at a nonexistent file and Android has no audio at all.
    #
    # Resolution order (GOW patch):
    #   1. PULSE_SERVER env var -- Wolf sets it to either
    #      "unix:$XDG_RUNTIME_DIR/pulse-socket" or the bare path. Strip the
    #      "unix:" scheme and use the path verbatim.
    #   2. $XDG_RUNTIME_DIR/pulse-socket -- Wolf's convention when
    #      PULSE_SERVER is not exported into the container.
    #   3. $PULSE_RUNTIME_PATH/native -- the upstream default.
    pulse_host_socket = ""
    pulse_server = os.environ.get("PULSE_SERVER", "")
    if pulse_server:
        if pulse_server.startswith("unix:"):
            pulse_server = pulse_server[len("unix:"):]
        # `unix:/path` sometimes has a leading `;`-joined list or authority;
        # take the first address that names a filesystem path.
        for cand in pulse_server.split(";"):
            cand = cand.strip()
            if cand.startswith("/"):
                pulse_host_socket = cand
                break
    if not pulse_host_socket:
        xdg = session["xdg_runtime_dir"]
        wolf_socket = os.path.join(xdg, "pulse-socket")
        if os.path.exists(wolf_socket):
            pulse_host_socket = wolf_socket
    if not pulse_host_socket:
    ...
    # The pulse socket is NOT load-bearing for Android boot: when the host
    # has no PulseAudio server (Wolf runs one only with WOLF_EMBED_PULSE=true)
    # the socket is absent and LXC otherwise aborts the whole container with
    # "Failed to mount ... pulse/native: No such file or directory".
    # Mark it optional so a missing socket degrades to "no audio" rather than
    # "no Android". (GOW patch -- see apps/waydroid.)
    make_entry(pulse_host_socket, pulse_container_socket[1:], options="rbind,create=file,optional 0 0")
