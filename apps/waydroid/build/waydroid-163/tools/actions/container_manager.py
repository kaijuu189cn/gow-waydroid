...
def restart(args):
    ...
    if status == "RUNNING":
        helpers.lxc.stop(args)
        helpers.lxc.start(args)
        # Clear any tracked session, exactly as stop() does.
        #
        # BUG THIS FIXES: restart() calls lxc.stop/lxc.start directly and so
        # bypasses stop(), which is the only place that does `del args.session`.
        # do_start() rejects a manager that still carries a session:
        #
        #     if "session" in args:
        #         raise RuntimeError("Already tracking a session")
        #
        # Because this manager process lives for the whole container lifetime,
        # a restart left args.session set and the NEXT session start failed with
        # "Already tracking a session", tearing the session down seconds after
        # it came up. Observed as the container exiting (code 143) right after
        # starting, with:
        #
        #     RuntimeError: Already tracking a session
        #     Gdk-Message: Error reading events from display: Broken pipe
        #
        # The in-container netfix path calls `waydroid container restart`, so
        # this fired on ordinary startups, not just manual restarts.
        if "session" in args:
            del args.session
    else:
