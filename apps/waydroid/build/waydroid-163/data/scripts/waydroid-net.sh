...
# Prefer the nft-backed iptables. On kernels built without the legacy
# netfilter tables (the case on this host: 7.2.4-arch1-2), `iptables-legacy`
# fails with "Table does not exist", while the nft backend works. Waydroid
# only needs *a* working nat/filter table, so prefer nft and fall back.
IPTABLES_BIN="$(command -v iptables-nft)"
[ -n "$IPTABLES_BIN" ] || IPTABLES_BIN="$(command -v iptables)"
[ -n "$IPTABLES_BIN" ] || IPTABLES_BIN="$(command -v iptables-legacy)"
IP6TABLES_BIN="$(command -v ip6tables-nft)"
[ -n "$IP6TABLES_BIN" ] || IP6TABLES_BIN="$(command -v ip6tables)"
[ -n "$IP6TABLES_BIN" ] || IP6TABLES_BIN="$(command -v ip6tables-legacy)"
